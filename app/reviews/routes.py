...
async def add_review(product_id: int, review: ReviewCreate, token: str = Depends(oauth2_scheme)):
    try:
        ...
        if not new_review:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Failed to create review")
        ...
        print(f"Exception: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error adding review")
...
async def get_reviews(product_id: int):
    try:
        ...
        print(f"Exception: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error fetching reviews")

@router.put("/products/{product_id}/reviews/{review_id}", response_model=ReviewResponse)
async def update_review(product_id: int, review_id: int, review: ReviewUpdate, token: str = Depends(oauth2_scheme)):
    try:
        payload = verify_token(token)
        username = payload.get("sub")
        ...
        cursor.execute("SELECT * FROM Reviews WHERE reviewId=? AND userId=?", (review_id, user_id))
        ...
        if not updated_review:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Failed to update review")
        ...
        print(f"Exception: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error updating review")
    

@router.delete("/products/{product_id}/reviews/{review_id}", status_code=status.HTTP_200_OK)
async def delete_review(product_id: int, review_id: int, token: str = Depends(oauth2_scheme)):
    try:
        payload = verify_token(token)
        username = payload.get("sub")
        ...
        print(f"Exception: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting review")
